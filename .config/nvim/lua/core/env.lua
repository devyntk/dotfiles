...
if vim.fn.filereadable(node_alias) == 1 then
    ...
    local node_bin =
        vim.fn.expand("$HOME/.nvm/versions/node/v" .. node_version .. "/bin")
...
local get_venv = function()
    ...
    local venv = vim.fs.find(
        { ".venv", "env" },
        { path = gitroot, limit = 1, type = "directory" }
    )
    if #venv == 1 then
    ...
    local setupcfg = vim.fs.find(
        { "setup.cfg" },
        { path = gitroot, limit = 1, type = "file" }
    )
    if #setupcfg == 1 then
